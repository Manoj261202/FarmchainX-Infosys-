...
export class AdminUsers implements OnInit {
  ...
  promote(userId: number): void {
    ...
    if (
      !user ||
      !confirm(
        `Are you sure you want to promote ${user.name} to Admin? This action cannot be undone.`,
      )
    )
      return;
