      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div *ngFor="let product of products()" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col group">
            ...
            <div class="h-52 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 relative">
                ...
                <!-- Quality Grade Badge -->
                <div class="absolute top-3 right-3 bg-white/95 backdrop-blur px-3 py-1.5 rounded-lg text-xs font-bold shadow-md"
            ...
            <div class="p-5 flex-1 flex flex-col">
                ...
                <div class="text-sm text-gray-600 mb-4 space-y-1 flex-1">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ product.harvestDate | date }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- QR Code -->
                <div class="flex items-center gap-3 mt-3 pt-3 pb-1" style="border-top: 1px solid #e5e7eb">
                  <img
                    [src]="getQrUrl(product.id)"
                    alt="QR"
                    class="w-12 h-12 rounded-lg flex-shrink-0"
                    style="background:#fff; padding:2px; border:1px solid #d1fae5"
                    (error)="$any($event.target).style.display='none'"
                  />
                  <div>
                    <p class="text-xs text-gray-400 mb-1.5">Product QR</p>
                    <div class="flex gap-1.5">
                      <a [href]="getQrUrl(product.id)" target="_blank" rel="noopener"
                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors">
                        View
                      </a>
                      <a [href]="getQrUrl(product.id)"
                        [download]="'qr-' + product.cropName + '-' + product.id + '.png'"
                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors border border-gray-200">
                        Save
                      </a>
                    </div>
                  </div>
